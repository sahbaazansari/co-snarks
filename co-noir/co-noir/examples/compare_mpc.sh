...
for f in "${test_cases[@]}"; do
 ...
 wait $(jobs -p)
    # run proving in MPC
  bash -c "cargo run --release --bin co-noir -- build-and-generate-proof --witness test_vectors/${f}/${f}.gz.0.shared --circuit test_vectors/${f}/target/${f}.json --crs test_vectors/g1.data --protocol REP3 --hasher POSEIDON --config configs/party1.toml --out test_vectors/${f}/proof --public-input public_input.json $PIPE"  || failed=1&
  bash -c "cargo run --release --bin co-noir -- build-and-generate-proof --witness test_vectors/${f}/${f}.gz.1.shared --circuit test_vectors/${f}/target/${f}.json --crs test_vectors/g1.data --protocol REP3 --hasher POSEIDON --config configs/party2.toml --out test_vectors/${f}/proof.1.proof  $PIPE"  || failed=1&
  bash -c "cargo run --release --bin co-noir -- build-and-generate-proof --witness test_vectors/${f}/${f}.gz.2.shared --circuit test_vectors/${f}/target/${f}.json --crs test_vectors/g1.data --protocol REP3 --hasher POSEIDON --config configs/party3.toml --out test_vectors/${f}/proof.2.proof $PIPE"  || failed=1
  wait $(jobs -p)
   # Create verification key
  bash -c "cargo run --release --bin co-noir -- create-vk --circuit test_vectors/${f}/target/${f}.json --crs test_vectors/g1.data --hasher POSEIDON --vk test_vectors/${f}/vk $PIPE"  || failed=1
  # verify proof
...
bash -c "cargo run --release --bin co-noir -- generate-witness --input test_vectors/${f}/Prover.toml.2.shared --circuit test_vectors/${f}/target/${f}.json --protocol REP3 --config configs/party3.toml --out test_vectors/${f}/${f}.gz.2.shared $PIPE"  || failed=1
 wait $(jobs -p)
  # run proving in MPC
bash -c "cargo run --release --bin co-noir -- build-and-generate-proof --witness test_vectors/${f}/${f}.gz.0.shared --circuit test_vectors/${f}/target/${f}.json --crs test_vectors/g1.data --protocol REP3 --hasher KECCAK --config configs/party1.toml --out test_vectors/${f}/proof --public-input public_input.json$PIPE"  || failed=1&
bash -c "cargo run --release --bin co-noir -- build-and-generate-proof --witness test_vectors/${f}/${f}.gz.1.shared --circuit test_vectors/${f}/target/${f}.json --crs test_vectors/g1.data --protocol REP3 --hasher KECCAK --config configs/party2.toml --out test_vectors/${f}/proof.1.proof  $PIPE"  || failed=1&
bash -c "cargo run --release --bin co-noir -- build-and-generate-proof --witness test_vectors/${f}/${f}.gz.2.shared --circuit test_vectors/${f}/target/${f}.json --crs test_vectors/g1.data --protocol REP3 --hasher KECCAK --config configs/party3.toml --out test_vectors/${f}/proof.2.proof $PIPE"  || failed=1
 wait $(jobs -p)
  # Create verification key
  bash -c "cargo run --release --bin co-noir -- create-vk --circuit test_vectors/${f}/target/${f}.json --crs test_vectors/g1.data --hasher KECCAK --vk test_vectors/${f}/vk $PIPE"  || failed=1
  # verify proof
